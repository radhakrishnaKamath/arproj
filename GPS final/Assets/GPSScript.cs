...
public class GPSScript : MonoBehaviour {
	...
	private GameObject seeTextObject;
	private GameObject actualDistanceTextObject;
		...
		while (true) {
			...
			if (Input.location.status == LocationServiceStatus.Failed) {
				...
				if (setOriginalValues) {
					originalLatitude = 19.065142f;
					originalLongitude = 73.009179f;
	...
	public void Calc(float lat1, float lon1, float lat2, float lon2)
	{
		...
		distanceTextObject.GetComponent<Text> ().text = "Lat: " + lat2 + " Lon: " + lon2;
		actualDistanceTextObject.GetComponent<Text> ().text = "Distance: " + distance;
		if(distance < 10.0000f){
			seeTextObject.GetComponent<Text> ().text = "Can I see? " + "Yes";
			//Model.SetActive (true);
		}
		else {
			seeTextObject.GetComponent<Text> ().text = "Can I see? " + "No";
	...
	void Start(){
		...
		actualDistanceTextObject = GameObject.FindGameObjectWithTag ("actualDistance");
		seeTextObject = GameObject.FindGameObjectWithTag ("seeText");
